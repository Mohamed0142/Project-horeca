        ...
        <div class="bg-white p-10 rounded-lg shadow-lg flex flex-col md:flex-row md:space-x-10">
            ...
            <form action="/contact" method="post" class="bg-white p-8 rounded-lg w-full md:w-1/2 max-w-lg shadow-md">
                ...
                <div class="mb-4">
                    ...
                    <input class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-yellow-200" type="text" id="phone" name="tel" value="<?php echo htmlspecialchars($postData['tel'] ?? ''); ?>">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1" for="email">Email</label>
                    <input class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-yellow-200" type="email" id="email" name="email" value="<?php echo htmlspecialchars($postData['email'] ?? ''); ?>">
                </div>
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-1" for="message">Your message</label>
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-yellow-200" id="message" name="message" rows="4"><?php echo htmlspecialchars($postData['message'] ?? ''); ?></textarea>
                </div>
